...
def get_served_list():
    try:
        with open('served.json') as served:
            data = json.load(served)
            return data
    except json.decoder.JSONDecodeError as e:
        print(f"Error loading served list: {e}")
        return None
    except FileNotFoundError:
        print("Served file not found.")
        return None
...
def download_chunk(chunk, transferred_data, size, s):
    with open("downloads/temp/" + chunk, 'wb') as f:
        ...
        while True:
            ...
            if not data:
                break
            f.write(data)
    print("\n<CLIENT>: Received file {0}, transferred data {1}".format(chunk, size_format(transferred_data)))


if __name__ == "__main__":
    served_list = get_served_list()
    if served_list is not None:
        now = datetime.datetime.now()
        timestamp = datetime.datetime.timestamp(now)
        logging.basicConfig(filename="RapidSport.xlsx", level=logging.INFO)
        ...
        transferred_data = 0
        while True:
            menu = int(input("Welcome to Menu. Please write the index:\n1. Download file\n2. Exit\n"))
            ...
            if menu == 1:
                ...
                while index < len(download_list):
                    ...
                    try:
                        ...
                        break
                    if filestatus == "1":
                        download_chunk(chunk, transferred_data, size, s)
                        ...
                        if count >= 1:
                            new = 1
                            while count >= 1:
                                print("File not found. Trying to download from a new peer.")
                                s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                                filestatus = ""
                                try:
                ...
                if index == len(download_list):
                    print("{0} is being combined. Please wait.".format(file_name))
                    combine(file_name)
                    d_path = os.path.dirname(__file__) + "/downloads"
                    print("File successfully combined. Combined file is located under: {0}".format(d_path))
            else:
                break
